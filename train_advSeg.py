...
def main():
    ...
    if args.labeled_ratio is None:
        ...
        trainloader_remain = paddle.io.DataLoader(train_dataset,
                         batch_sampler=train_remain_sampler, num_workers=4)
        trainloader_gt = paddle.io.DataLoader(train_dataset,
                       batch_sampler=train_gt_sampler, num_workers=4)
    ...
    for i_iter in range(args.num_steps):
        ...
        if (args.lambda_semi > 0 or args.lambda_semi_adv > 0 ) and i_iter >= args.semi_start_adv :
            ...
            loss_semi_adv = args.lambda_semi_adv * bce_loss(paddle.cast(D_out,paddle.float32),make_D_label(gt_label, ignore_mask_remain))
            ...
            if args.lambda_semi <= 0 or i_iter < args.semi_start:
                ...
                # produce ignore mask
                semi_ignore_mask = (D_out_sigmoid < args.mask_T) # TODO fix
        ...
        loss_seg = loss_calc(pred, labels) # Cross entropy loss for labeled data

        D_out = interp(model_D(F.softmax(pred,axis=1))).astype(paddle.float32)

        D_label = make_D_label(gt_label, ignore_mask).astype(paddle.float32)

        loss_adv_pred = bce_loss(D_out, D_label)
        ...
        D_out = interp(model_D(F.softmax(pred,axis=1))).astype(paddle.float32)
        D_label_gt = make_D_label(pred_label, ignore_mask).astype(paddle.float32)

        loss_D = bce_loss(D_out, D_label_gt)
        loss_D = loss_D/2.0
        loss_D.backward()
        ...
        D_label_mask_gt = make_D_label(gt_label, ignore_mask_gt).astype(paddle.float32)

        loss_D = bce_loss(D_out, D_label_mask_gt)
        loss_D = loss_D/2.0
        loss_D.backward()
        loss_D_value += loss_D.cpu().numpy()[0]
        ...
        if i_iter >= args.num_steps - 1:
            print('save model ...')
            paddle.save(model.state_dict(), os.path.join(args.checkpoint_dir, str(args.num_steps) + '.pth'))
            paddle.save(model_D.state_dict(), os.path.join(args.checkpoint_dir, str(args.num_steps) + '_D.pth'))
            break

        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            print('saving checkpoint  ...')
            paddle.save(model.state_dict(), os.path.join(args.checkpoint_dir, str(i_iter) + '.pth'))
            paddle.save(model_D.state_dict(), os.path.join(args.checkpoint_dir, str(i_iter) + '_D.pth'))
