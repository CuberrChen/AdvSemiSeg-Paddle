...
def update_ema_variables(model, ema_model, alpha, global_step):
    ...
    for ema_param, param in zip(ema_model.parameters(), model.parameters()):
        tmp_param = paddle.multiply(param, paddle.to_tensor(alpha).astype('float32'))
        tmp_param = paddle.add(tmp_param, paddle.multiply(paddle.to_tensor(1 - alpha), param))
        ema_param.set_value(tmp_param)
