...
class ContextSegmentation(BaseDataset):
    ...
    def __init__(self, root, split='train',
        ...
        if os.path.exists(mask_file):
            self.masks = paddle.load(mask_file)
        else:
    ...
    def __getitem__(self, index):
        ...
        # general resize, normalize and toTensori
        #mask.save('./pc_gt_train/'+img_id['file_name'])
        ...
        return img, mask, size , size , size#, img_id['file_name']

    def _mask_transform(self, mask):
        #target = np.array(mask).astype('int32') - 1 # for 59 classes
        target = np.array(mask).astype('int32') # for 60 classes
        
        return paddle.to_tensor(target).astype('int64')
